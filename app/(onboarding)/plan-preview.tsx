    ...
    >
      ...
      <Card style={{ paddingTop: 18 }}>
        <View style={{ flexDirection: 'row', gap: 12 }}>
          <Stat label="Weeks" value={race.totalWeeks} color={hues.sky.text} size="md" />
          <Stat label="Runs" value={profile.runDaysPerWeek} unit="/wk" color={hues.amber.text} size="md" />
          <Stat label="Peak" value={Math.max(...sampleBlockMiles)} unit="mi" color={hues.accent.text} size="md" />
        </View>
      </Card>

      <Card gap={12}>
        <CardHeader icon={Flag} title="Weekly miles" hue={hues.accent} meta={`${race.totalWeeks} weeks to race day`} />
        <BlockChart miles={sampleBlockMiles} phases={sampleBlockPhases} height={190} showPhases />
      </Card>
